...
def signal_constellation_generator(modulation_type, plot=False):
    ...
    match modulation_type:
        ...
        case modulation_type if modulation_type[-3:] == "PSK" and modulation_type[0].isnumeric():
            number_of_symbols = int(modulation_type[:-3])
            constellation = np.exp(1j*2*np.pi*np.arange(0, number_of_symbols)/number_of_symbols )
        case "MSK" | "GMSK":
...
def qam_map_generator(number_of_symbols, plot=False):
    ...
    Points are normalized to unit average power (E[|s|²] = 1), consistent
    with PSK constellations and standard practice (MATLAB qammod, GNU Radio).
    ...
    Returns:
    numpy.ndarray: Complex constellation points with unit average power
    """
    ...
    # Edge case for 8QAM (cross-shaped, not a square grid)
    if number_of_symbols == 8:
        constellation = np.array([1, 1+1j, 1j, -1+1j, -1, -1-1j, -1j, 1-1j], dtype=complex)

    # For square QAM (e.g., 4, 16, 64, 256)
    elif np.mod(np.sqrt(number_of_symbols), 1) == 0:
    ...
    # Normalize to unit average power — standard practice for QAM
