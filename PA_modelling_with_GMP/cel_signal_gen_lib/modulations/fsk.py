...
def create_gmsk_signal(number_of_symbols, samples_per_symbol, f_carrier=0.05, BT=0.3, gauss_filter_span=4, conv_mode='same', plot=False, normalize=False, rng=np.random.default_rng()):
    """
    ...
    MSK_symbol_gauss = scipy.signal.oaconvolve(MSK_symbol_upsampled, gaussian_design(BT, samples_per_symbol, gauss_filter_span, normalize=normalize), mode=conv_mode)
