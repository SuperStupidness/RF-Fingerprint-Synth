...
def create_rect_bpsk_signal(number_of_symbols, samples_per_symbol, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    BPSK_signal = scipy.signal.oaconvolve(BPSK_symbols_upsampled, rect_pulse_shape, mode=conv_mode)
...
def create_srrc_bpsk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    BPSK_signal = scipy.signal.oaconvolve(BPSK_symbols_upsampled, SRRC_pulse_shape, mode=conv_mode)
...
def create_rc_bpsk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    BPSK_signal = scipy.signal.oaconvolve(BPSK_symbols_upsampled, RC_pulse_shape, mode=conv_mode)
...
def create_rect_qpsk_signal(number_of_symbols, samples_per_symbol, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    QPSK_signal = scipy.signal.oaconvolve(QPSK_symbols_upsampled, rect_pulse_shape, mode=conv_mode)
...
def create_srrc_qpsk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    # Apply SRRC pulse shaping
    SRRC_pulse_shape = srrc_design(samples_per_symbol, filter_span, beta)
    QPSK_signal = scipy.signal.oaconvolve(QPSK_symbols_upsampled, SRRC_pulse_shape, mode=conv_mode)
...
def create_rc_qpsk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    # Apply SRRC pulse shaping
    RC_pulse_shape = rc_design(samples_per_symbol, filter_span, beta)
    QPSK_signal = scipy.signal.oaconvolve(QPSK_symbols_upsampled, RC_pulse_shape, mode=conv_mode)
...
def create_rect_8psk_signal(number_of_symbols, samples_per_symbol, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    PSK_signal = scipy.signal.oaconvolve(PSK_symbols_upsampled, rect_pulse_shape, mode=conv_mode)
...
def create_srrc_8psk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    # Apply SRRC pulse shaping
    SRRC_pulse_shape = srrc_design(samples_per_symbol, filter_span, beta)
    PSK_signal = scipy.signal.oaconvolve(PSK_symbols_upsampled, SRRC_pulse_shape, mode=conv_mode)
...
def create_rc_8psk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    # Apply SRRC pulse shaping
    RC_pulse_shape = rc_design(samples_per_symbol, filter_span, beta)
    PSK_signal = scipy.signal.oaconvolve(PSK_symbols_upsampled, RC_pulse_shape, mode=conv_mode)
...
def create_rect_pi4_dqpsk_signal(number_of_symbols, samples_per_symbol, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    DQPSK_signal = scipy.signal.oaconvolve(DQPSK_symbols_upsampled, rect_pulse_shape, mode=conv_mode)
...
def create_srrc_pi4_dqpsk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    SRRC_pulse_shape = srrc_design(samples_per_symbol, filter_span, beta)
    DQPSK_signal = scipy.signal.oaconvolve(DQPSK_symbols_upsampled, SRRC_pulse_shape, mode=conv_mode)
...
def create_rc_pi4_dqpsk_signal(number_of_symbols, samples_per_symbol, filter_span, beta, conv_mode='same', plot=False, rng=np.random.default_rng()):
    """
    ...
    RC_pulse_shape = rc_design(samples_per_symbol, filter_span, beta)
    DQPSK_signal = scipy.signal.oaconvolve(DQPSK_symbols_upsampled, RC_pulse_shape, mode=conv_mode)
